...
Claim states: `current`, `proposed`, `superseded`, `retracted`, `conflicted`, `unconfirmed`. `current` means currently adopted, not objectively verified. Add `subject` and `scope` when relating decisions. A change may include `claimRelations: [{anchor, relation, target: {articleId, revision, anchor}, evidence}]`; relations are `supersedes`, `retracts`, `contradicts`, `supports`. The target must be from an earlier publication. Subject/scope must match exactly and be nonempty. Relation evidence must be exact evidence already attached to the new claim. Only a current user decision can supersede/retract a user decision. Historical versions remain immutable.

`agent-wiki search "keywords" --view history` includes retired claims and fixed-version relation links; default `current` excludes superseded/retracted claims. Proposed, conflicted and unconfirmed claims retain explicit states. `--scope` filters exact scope (and unscoped records). Retrieval traverses at most two relation hops and reports truncation; follow fixed article/source links when needed.
