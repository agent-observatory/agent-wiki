...
- 사용자 명령 없이 정제 시작·중지하지 않는다. 사용자 요청한 Alibaba 무료 할당량 소진 안전 중지는 예외이며, 사용자가 설정한 2번 모델(`fallbackModel`)로의 전환만 자동이다. 그 밖의 모델·제공자 전환과 자동 재개는 하지 않는다. 배포·설정 저장·Hello·페이지 재조립은 enabled를 변경하지 않는다.
