...
- 정제 큐는 PostgreSQL 작업 테이블의 상태·5분 임대로 관리한다. 모델 호출은 동시성 1·같은 호스트/API 키당 최소 3초 간격이며 결과 폴링도 포함한다. 일시 오류는 키 전체의 backoff+jitter·제공자 Retry-After와 일일 한도를 지키며 계속 재시도한다. 키별 대기 시각과 청크 진행 위치는 DB에 보관한다. 인증·입력·근거 오류는 확인 필요 상태로 남긴다. Collector 기록 위치와 정제 작업 상태를 구분하고 결과 커밋 뒤 완료, 멱등 반영, 종료 유예를 지킨다. 로그만 남기고 작업 오류를 삼키지 않는다.
