...
- 세션은 여러 원천 자료 중 하나다. 주 용례는 에이전트가 질답·작업 중 근거 자료를 조회하는 것이다. 웹 위키는 지식·근거·검토 결과·진행 상태를 읽는 뷰어다. 모든 제품 변경·검토 확정·AI 설정·정제 제어는 CLI에서 실행한다. 검색 API와 인용 가능한 Context를 우선 설계하며 웹 방문량을 제품 가치의 기준으로 삼지 않는다.
...
- OCI A1 Compute VM 1대·2 OCPU·12GB는 유지한다. Compose는 Caddy·Next.js·API·Worker·PostgreSQL 5개다. CPU는 공유하고 Worker만 최대 0.5 CPU·2GB이며 BYOK는 Workspace별 최대 5로 제한한다.
