...
for (const variant of ["off", "high"])
  test(`captured Alibaba ${variant} output passes Worker publication with preserved provider history`, async () => {
    ...
    await tx(owner, ws, async (c) => {
      ...
      await c.query(
        "INSERT INTO sources(id,workspace_id,name,kind,origin,content_hash,payload_hash,object_key,line_count,idempotency_key,masked) VALUES($1::uuid,$2,'Synthetic','conversation',$1::text,$3,$3,$4,10,$1::text,true)",
        [id, ws, hash(text), key],
      );
      await c.query(
