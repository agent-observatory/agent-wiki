...
사용자는 Workspace별 OpenAI 호환 BYOK로 Alibaba Qwen을 선택할 수 있다. 서버 기본 모델과 별개이며 정확한 API 호스트를 서버 허용 목록에 등록한다. Alibaba Qwen 3.5–3.8 Flash·Plus·Max의 추론 사용 안 함은 `enable_thinking: false`, 기본값은 옵션 생략이다. 세부 추론 수준은 지원하지 않는다. [공식 추론 옵션](https://www.alibabacloud.com/help/en/model-studio/deep-thinking).
