...
![L1부터 L3까지 한 장. 01 세션 증분을 Worker가 청킹·입력 조립·AI 추출·서버 검증·반영으로 정제한다. 입력 조립은 세션 메타데이터·암호화 본문·지침과 함께 Wiki CLI 호출과 그 출력 블록을 비워 위키가 자기 출력을 다시 먹는 순환을 끊는다. L3 현재 주장의 BM25 후보를 입력 조립에 되돌리고, 출력 오류는 같은 청크를 다시 보내며, 어긋난 관계는 그 관계만 떼어 통합 대기함에 넘긴 뒤 주장은 그대로 반영한다. 대기함은 두 상태다. 대상 Version 변경 409처럼 나중에 유효해질 수 있는 관계는 pending으로 남아 다음 통합이 모델에 다시 보내고, 양끝 subject·scope 불일치·출발 주장이 갖지 않은 근거·강등된 주장의 대체처럼 게이트가 절대 받을 수 없는 관계는 needs_human으로 남아 모델로 돌아가지 않고 review conflicts와 웹 Curation 탭에서 사람이 판단한다. 02 통합 전 L3에는 정본 subject ai-provider·scope general에 current 주장 A·B가 함께 남고 제안 D와 대기함의 관계가 있다. B의 slug는 curation-provider인데 사람이 subject alias로 ai-provider에 이어 두 주장이 비교된다. 후보 계산은 모델 없이 하고 주장에 저장된 slug는 바꾸지 않으며 관계 게이트·통합 묶음 키·어휘 힌트·페이지 조립이 정본 이름으로 비교한다. 03 Consolidation Job은 cycle 완료·대기함 관계·수동 consolidate로 만들어지고 주제당 열린 Job은 1개이며, gather가 subject·scope별로 묶어 current가 둘 이상인 묶음만 후보로 삼되 묶음 지문이 직전 완료 Job과 같고 대기함이 비었으면 모델을 호출하지 않는다. model은 요청 크기를 재어 maxInputTokens 안에 드는 묶음까지만 보내고 남은 묶음은 지문을 기록하지 않아 미판단으로 남기며 후속 Job이 이어받는다. validate → publish가 관계만 판단해 자동 반영하고 Step별 상태를 따로 기록한다. 04 Wiki Page 새 Version은 현재 주장만 subject별로 나열하고 supports로 이어진 묶음은 코드가 고른 대표 하나로 접어 한 번만 실으며 클릭한 주장의 리니지 패널이 B가 A를 대체한 관계와 이유를 보여준다. contradicts는 양끝이 모두 current일 때만 conflicted를 만든다. 검토의 review conflicts는 needs_human 관계를 사람에게 보여 별칭·관계·철회 중 하나를 고르게 하고, relation reject는 정정 Version을 발행하고 거절을 기억한다. 카드 아래의 데이터베이스 기호 줄은 읽거나 쓰는 테이블이다](assets/wiki-l1-l3-curation.svg)
...
| `model` | gather 결과 하나로 BYOK 1회. 묶음마다 `supersedes`·`retracts`·`supports`·`contradicts`·`leave_unresolved(이유)` 중에서 제안한다. 새 주장·본문·상태 필드 변경은 출력 계약 위반이다. `supports`는 출발 주장이 대상과 **같은 진술**을 다시 말하거나 그 진술의 새 근거일 때만이다. 한 subject에 대한 서로 다른 참인 사실 둘은 supports가 아니라 `leave_unresolved(parallel)`이다. 관계 근거는 출발 주장의 기존 근거 ID 중에서 고른다. 대기함의 관계와 거절된 관계는 참고 정보로 함께 전달한다. 직전 completed Job이 이미 판단한 묶음은 다시 보내지 않는다: 묶음마다 (live 주장 키·상태, 기존 관계, 대기함 ID)의 지문을 `steps.gather.output.groupHashes`에 남기고, 지문이 같고 대기함이 비었으면 후보에서 뺀다. 주장·관계·대기함 항목이 하나라도 생기면 지문이 바뀌어 다시 열린다. 수동 실행도 이 기억을 따른다 | BYOK 1회. 요청 전체를 재서 `maxInputTokens` 안에 들어가는 묶음까지만 보낸다. 남은 묶음은 지문을 기록하지 않아 미판단으로 남고 후속 Job이 이어 받는다. 묶음 하나가 혼자 예산을 넘으면 그대로 보낸다 — 묶음 안을 쪼개면 같은 subject의 나머지 없이 한 쌍을 판단하게 되어 더 나쁘다 | 일시 제공자 오류(`429`·5xx·연결·시간 초과)는 추출과 같은 키 대기·`Retry-After`+5초 규칙. 출력 오류(잘못된 JSON·알 수 없는 주장 ID·허용 외 관계·근거 ID 불일치)는 캐시를 비우고 새 응답, 같은 Job·세대에서 3회 연속이면 확인 필요. 어느 경우도 gather를 다시 하지 않으며 재시도는 `steps.model.output`에 있는 gather 결과를 그대로 다시 쓴다. 그 결과가 없으면 모델을 부르지 않고 gather부터 다시 한다 |
...
웹은 이미 받는 Wiki Page Version 스냅샷(`claims`·`relations`·`references`)을 마크다운 대신 구조로 그린다. 새 API·새 저장은 없다. 스냅샷 안의 필드는 늘 수 있고(`clusters`는 위 접기 결과를 주장 ID로만 담는다) 웹은 그 결정을 그릴 뿐 규칙을 다시 계산하지 않는다. 마크다운의 `## Decision History` 절과 반영 이력 페이지(발행 이벤트의 시간순 목록)는 그대로 둔다.
...
- 주장을 클릭하면 오른쪽 패널(데스크톱 전용)이 리니지를 계산한다. 스냅샷 안에서 `supersedes`·`retracts`를 뒤(이 주장이 대체한 것)와 앞(이 주장을 대체한 것)으로 따라간다. 후속이 둘이면 분기로 둘 다 표시하고 오류로 보지 않는다. `contradicts`는 배지다. `supports`로 이어진 `current` 주장들은 **연결 성분 하나를 대표 하나로 접어** 목록과 페이지 본문에 한 번만 싣고 나머지는 "재확인" 링크로 둔다. 대표 선정은 완전히 결정적이다: 권한(`user_decision` → `author_statement` → `observation` → `ai_inference` → `agent_statement`) → 다른 주장을 supports하지 않는 것(재진술이 아닌 원 진술) → 가장 이른 `recorded` 근거 시각(`recovered`·미확인은 뒤) → 근거 수 → 주장 ID. 행 순서와 무관하게 같은 결과가 나온다. 모델은 관계를 제안하고 접는 규칙은 엔지니어링이 갖는다. 관계마다 근거·publication 종류(추출·통합·수동)·시각을 보여주고, 거절된 관계는 취소선과 정정 Version 링크로 표시한다. 선형 이력을 강제하지 않는다.
...
**게이트가 절대 받을 수 없는 관계는 대기함에 `needs_human`으로 남긴다.** 대기함의 `pending`은 "아직 판단하지 않았다"는 뜻이라 다음 Consolidation이 모델에 다시 보낸다. 그런데 양끝의 subject·scope가 어긋났거나 출발 주장이 갖지 않은 근거를 인용한 관계는 **나중에도 유효해지지 않는다** — 다시 보내면 같은 제안이 돌아오고 호출만 탄다. 그래서 그 관계만 버려 주장을 살리되(그림 01) 버린 사실을 `needs_human` 행으로 남긴다. gather는 `pending`만 읽으므로 모델에는 가지 않고, `review conflicts`와 웹 Curation 탭이 양끝 주장·subject·scope와 함께 **이유별 결정적인 행동 한 줄**을 보여준다: 범위 불일치이고 subject가 다르면 `subject alias add`, 근거 불일치면 `relation add`, 강등된 주장의 제안이면 `claim retire`·`claim assert`. 사람이 `relation add`로 같은 관계를 실제로 이으면 그 행은 `resolved`가 된다. 폐기 수 자체는 실행 진단(`droppedRelations`)에도 남아 둘을 대조할 수 있다.


L2 입력에서 확인된 세션 메타데이터 이벤트·암호화 본문·기본 시스템/Skill/개발자 지침 필드를 제외한다. `text-fields-5`는 provenance·ID와 명시적인 lineage 이벤트를 AI 본문에서 빼고, 명시적인 메시지 `role=system/developer`도 제외하며, **Wiki CLI를 실행한 도구 호출과 그 출력 블록 전체**를 제외한다(`wiki_echo`). 위키가 자기 출력을 다시 읽어 지식으로 올리는 순환은 역할 게이트로 못 막는다 — 도구 출력은 정당한 관찰이 인용하는 바로 그것이고, `kubectl get pods`와 `agent-wiki pages`를 역할로는 구분할 수 없다. 그래서 판정이 가능한 유일한 지점인 **명령**에서 끊는다. 짝은 `id`·`call_id`·`tool_use_id`로 맞춘다. 중첩 스냅샷에서는 해당 메시지만 제외하며 사용자 메시지·본문에 인용된 지침·역할 미확인 내용은 유지한다. 세션 ID·에이전트 별명·시작 시각을 지식으로 만들지 않는다. L1은 그대로 두고 해당 줄만 비워 원문 줄 번호를 유지한다. 생략 범위는 입력·실행 기록에 남기며 그 범위를 걸친 인용은 거부한다. 제외 필드만 있는 청크는 모델을 호출하지 않는다. Collector가 L1 전에 실행 메타데이터와 중복 컴팩션 기록을 선별한다. L2는 남은 원문의 의미를 임의로 지우지 않는다. 컴팩션의 중첩 메시지는 각각의 발화자를 구분하고, 도구 실행 요청과 실제 출력도 분리한다.
