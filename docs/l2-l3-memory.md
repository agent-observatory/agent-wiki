...
`remote-curation-3`는 동일 세션에서 이미 반영한 주장과 키워드 유사도를 우선해 최대 6개 주장을 참고한다. 제목·본문·상태·대상·적용 범위·고정 근거 위치를 포함하며 1,800 토큰 추정량 안에 넣는다. 한국어·코드 입력은 UTF-8 바이트 수를 보수적인 토큰 상한으로 사용한다. 참고 내용을 새 주장의 원문 근거로 인용하지 않는다.
...
- 초기 재생성 단위는 **Workspace 전체**다. 여러 세션이 같은 지식을 갱신할 수 있어 특정 세션의 지식만 단순 삭제하지 않는다. 부분 재생성은 의존 주장·맥락의 영향 범위를 계산한 뒤 확장한다.
- L1 객체·원문 ID·이벤트/수집 출처·Collector 수신 위치·Workspace·인증·AI 연결 설정을 유지한다. 새 L1 수집도 계속한다. 시작 시 재생성할 확정 입력 목록을 고정하고 그 뒤의 새 증분은 다음 처리 범위로 둔다.
- 정제 신규 수신을 멈추고 진행 실행을 종료한다. 실행 세대를 증가시켜 이전 Worker 응답·재시도가 뒤늦게 반영되지 않도록 커밋 시 세대를 검사한다. API·조회는 초기화/재생성 중 상태를 구분해 반환한다.
- **Knowledge를 지울 때는 해당 Curation 결과·세션 맥락·정제 처리 위치·시작 Context·조회 캐시도 함께 무효화한다.** 기존 완료 위치를 남겨 새 실행이 원문을 건너뛰지 않게 한다. 보존 L1로 작업을 다시 구성하고 새 세대의 멱등 키를 사용한다.
- 지식이 남아 있다면 그 지식의 필수 정제 입력·실행·근거 이력만 먼저 지워 리니지를 끊지 않는다. 진단 로그만 정리하는 작업과 L2/L3 초기화를 구분한다. 지식과 연결된 정제 이력을 없애려면 연결된 파생 지식을 함께 재생성한다.
- 재생성은 새 Version/ID·새 실행 기록을 만들며 옛 링크를 새 의미로 재사용하지 않는다. 이전 파생 데이터 삭제를 선택한 경우 그 데이터의 링크는 사용 불가로 반환한다. 재정제 입력·규칙·모델·시작/완료 범위는 새 실행에 기록한다.
- 초기화 실패는 재생성 단계에 남겨 재개하며 수집 위치를 되돌리지 않는다. AI 활성 상태를 초기화만으로 켜지 않는다. 명시적인 재정제 실행 요청과 중지/재개 설정을 따른다.
