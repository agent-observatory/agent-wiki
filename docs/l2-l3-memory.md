...
관계는 **주장 Version 사이**에 저장한다. 관계의 방향·종류·근거·생성 실행을 보존하고 자기 참조·대체 순환을 거부한다. 발언 시각·실제 적용 시점·서버 수집/반영 시각을 분리한다. 적용 시점이 명시되지 않으면 미확인으로 두며 추측으로 채우지 않는다. 하나의 관계로 페이지 전체를 폐기하거나 언급되지 않은 기존 주장을 지우지 않는다. 새 개정에는 변경하지 않은 주장과 기존 인용을 유지한다.
...
일반 조회는 요청한 범위·시점의 유효 상태를 확인해 현재 결정을 반환한다. 과거 주장이 검색에 걸리면 대체 관계를 따라 현재 주장을 확인한다. 현재 주장을 못 읽거나 관계가 불명확하면 과거 주장을 현재 사실처럼 대신 반환하지 않는다. 변경 이유를 묻는 조회에는 과거 주장과 변경 근거를 함께 담고, 미해결 충돌은 임의로 한쪽을 고르지 않는다.
...
기본 목록은 **세션 한 줄 + 대기·진행 중·실패·완료·청크 반영**이다. 서버에서 세션별 집계 후 페이지를 나누며 작업 수와 세션 수를 구분한다. 재시도 대기는 대기에 포함해 보조 표시한다. 개별 작업의 펼침 목록은 제공하지 않는다. 청크·시도·실패 상세는 내부 진단 기록으로 보존하고 최근 호출 이력은 기존 화면에서 확인한다. 작업 상태와 청크 진척도는 별개이고 미분할 자료가 있으면 완료율로 숨기지 않는다.
