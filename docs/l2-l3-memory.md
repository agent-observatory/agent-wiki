...
`remote-curation-10`의 `workspace-topic-relevance-2`는 **에이전트 종류와 무관하게 Workspace 전체의 대상·적용 범위·본문·별칭을 비교**한다. 전송 JSON 대신 디코딩한 대화 본문을 사용해 청크 뒤쪽의 주제도 후보 검색에 포함한다. 관련도가 같을 때 동일 세션을 우선한다. 후보 24개에서 최대 6개·1,800바이트 예약 예산으로 제한하며 원문·출력 예산을 침범하지 않는다. 검색 관련도는 병합 판단이 아니다. 선택한 주장·Version·상태·원문 위치와 정책 버전을 실행 입력·진단에 보존한다. 참고 지식은 새 주장의 원문 근거가 아니다.
