...
`remote-curation-6`의 맥락 선택 규칙 `session-anchor-relevance-1`은 **동일 세션의 관련 주장 1개를 먼저**, 나머지는 Workspace 전체의 본문 유사도 순으로 고른다. 같은 세션의 다른 주장이 예산을 모두 차지해 다른 세션의 관련 결정을 밀어내지 않도록 한다. 후보 24개에서 최대 6개를 선택하고 예산을 넘는 후보는 건너뛴다. 동일 세션 후보가 없으면 전체 관련도 순으로 선택한다. 문자열 관련도는 의미·정정 의도를 보장하지 않으며 실제 관계는 기존 원문·권한·범위 검사로 판정한다. 제목·본문·상태·대상·적용 범위·고정 근거 위치를 포함하며 1,800 토큰 추정량 안에 넣는다. 한국어·코드 입력은 UTF-8 바이트 수를 보수적인 토큰 상한으로 사용한다. 실행 진단에는 선택 규칙 버전·선택 수·동일 세션 수·입력 바이트·예산을 남기고 실제 선택 주장과 Version은 기존 실행 입력에 보존한다. 참고 내용을 새 주장의 원문 근거로 인용하지 않는다. 서버가 L1의 역할 필드로 발화자를 확인한다. 입증되지 않은 사용자 결정·관찰은 미확인으로 보존하며 자동 정정 권한을 부여하지 않는다. 역할 정보가 조각 밖에 있으면 사용자 발언으로 추정하지 않는다.

모델 응답의 단일 행 표기 `[159]`는 `[159,159]`로 정규화한다. 양의 정수 한 개만 허용하며 공개 API 계약과 원래 응답은 바꾸지 않는다. 모델이 JSON 필드 내부의 줄 번호를 셀 수 있으므로, 인용문이 **현재 청크의 디코딩된 본문에 정확히 한 번 일치**하면 서버가 불변 L1 행과 전체 원문 인용으로 연결한다. 예: 보관 행 159 내부의 본문 10번째 줄도 근거 위치는 L1 행 159다. 다른 원문·개정, 중복 일치, 공백/대소문자가 다른 유사 인용, 참고 맥락만의 일치는 보정하지 않는다. 연결 뒤에도 범위·생략 구간·발화자·정정 권한·원문 일치 검사를 적용한다. L1과 원래 모델 응답은 보존하고 연결 횟수는 `anchoredEvidence`로 기록한다. 새 문서의 임시 `clientRef`·주장 `anchor`가 응답 안에서 반복되면, 참조되지 않는 중복만 충돌 없는 이름으로 구분한다. 링크·관계가 중복 이름을 가리키거나 기존 문서를 편집하는 경우에는 자동으로 뜻을 배정하지 않는다. 주장·인용은 그대로 두며 변경 수를 실행 진단에 기록한다. L2 호출 이력에는 인용의 원문 위치·내용 일치 개수를 표시한다. 인용 일치는 주장의 진실성이나 정정 권한 검증 완료를 뜻하지 않으며, 이전 기록의 미집계는 0으로 표시하지 않는다.

L2 입력에서 확인된 세션 메타데이터 이벤트·암호화 본문·기본 시스템/Skill/개발자 지침 필드를 제외한다. 세션 ID·에이전트 별명·시작 시각을 지식으로 만들지 않는다. L1은 그대로 두고 해당 줄만 비워 원문 줄 번호를 유지한다. 생략 범위는 입력·실행 기록에 남기며 그 범위를 걸친 인용은 거부한다. 제외 필드만 있는 청크는 모델을 호출하지 않는다. 일반 대화 속 같은 단어·알 수 없는 필드·컴팩션 대화 스냅샷은 임의 삭제하지 않는다. 컴팩션의 중첩 메시지는 각각의 발화자를 구분하고, 도구 실행 요청과 실제 출력도 분리한다.
...
L4 일반 조회는 검색어별 **일치 개수 → 필드 점수 → 갱신 시각·ID**로 정렬한다. 검색어 하나당 제목 4, 태그·별칭 3, 본문·용어집 확장 1 중 가장 높은 점수만 더한다. 반복 등장으로 점수를 부풀리지 않으며 페이지 분할 전에 정렬한다. 한국어 조사는 원래 표현과 제한적인 조사 제거형을 하나의 검색어로 묶고, 본문 발췌도 같은 대안을 사용한다. 긴 본문은 반환 길이의 1/4 간격으로 겹치는 구간과 검색어 첫 일치 앞 구간을 비교해 서로 다른 검색어가 많이 포함된 쪽을 고른다. 반복 횟수는 점수를 늘리지 않으며 동률이면 첫 일치 앞에 최대 240자 맥락이 있는 구간을 우선한다. 이는 제한된 구간 비교이며 문장의 의미·완전성을 보장하지 않는다. 반환 문자 예산·원문 기준 위치·잘림 표시는 유지한다. 이 방식은 형태소 분석·의미 검색이 아니다. 일반 조회는 추가 모델을 호출하지 않는다. 현재 상태·범위·대체 관계 판정은 관련도 점수와 별개로 유지한다. [조회 그림](architecture.md#l4--query) · [합성 평가](../experiments/retrieval/README.md).
