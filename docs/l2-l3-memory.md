...
`remote-curation-11`은 새 본문의 기존 결정을 `bm25-field-diversity-1`로 찾는다. 디코딩한 청크 전체와 Workspace의 현재 주장(제목·본문·대상·범위·별칭)을 비교한다. **BM25 → 후보 24개 → 대상·범위·별칭 및 주제 다양성으로 규칙 재정렬 → 최대 6개·1,800바이트 예산** 순서다. 근거 위치·Version·정책·후보 점수·선택 이유를 실행에 보존한다. 참고 지식은 새 주장의 원문 근거가 아니다. 긴 주장은 일부만 보내고 `textTruncated`를 표시한다.
...
기본은 **한 번의 추론 호출에서 추출과 관계 비교**다. 호출 분리는 필수 조건이 아니다. 추론 ON/OFF와 단계별 모델 선택은 별도 축이며, 현재 Alibaba DeepSeek Flash는 `enable_thinking=true`, `reasoning=high`, `thinking_budget=null`을 사용한다. 전체 생성은 `max_completion_tokens`로 제한한다. 작은 비교의 결과·한계는 [실험 기록](../experiments/curation/lineage/README.md)에 둔다.
