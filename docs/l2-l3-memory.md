...
`remote-curation-5`는 동일 세션에서 이미 반영한 주장과 현재 청크 본문의 키워드 유사도를 우선해 최대 6개 주장을 참고한다. 제목·본문·상태·대상·적용 범위·고정 근거 위치를 포함하며 1,800 토큰 추정량 안에 넣는다. 한국어·코드 입력은 UTF-8 바이트 수를 보수적인 토큰 상한으로 사용한다. 참고 내용을 새 주장의 원문 근거로 인용하지 않는다. 서버가 L1의 역할 필드로 발화자를 확인한다. 입증되지 않은 사용자 결정·관찰은 미확인으로 보존하며 자동 정정 권한을 부여하지 않는다. 역할 정보가 조각 밖에 있으면 사용자 발언으로 추정하지 않는다.
