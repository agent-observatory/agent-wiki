...
아래 한 장은 위에서 아래로 **01 증분 정제와 결정 이력, 02 방법을 고르는 실험, 03 원문을 유지하는 재생성, 04 에이전트 검토**다.

![01 두 세션의 증분을 청킹·입력 조립·AI 판단·서버 검증으로 정제하고 L3 현재 주장의 BM25 후보를 규칙 재정렬 뒤 입력 조립에 되돌린 다음 변경 의도를 구분해 L3에 주장 A·B와 대체 관계를 보존하고 L4가 질문별로 반환하는 흐름, 02 같은 평가 자료로 청킹·세션 맥락·규칙 재정렬·모델 리랭커를 하나씩 비교, 03 L1과 수집 위치를 유지하고 L2·L3 파생 상태를 새 실행에서 재생성, 04 에이전트 검토와 개념 단위 변경](assets/wiki-l2-l3-memory.svg)

01의 정제 Provider 변경은 합성 예시다. 새 증분·세션 맥락·기존 지식을 함께 읽고 검증한 뒤 다음 처리 상태를 남기며, L2가 변경 의도를 판단하고 L3가 과거·현재 주장을 보존하고 L4가 현재 결정·변경 이유·충돌을 구분해 조회한다.

02의 실제 원문으로 실행하는 기준선·평가 질문·초기화 전 결과 보존은 [실제 원문 정제 평가](../experiments/curation/production/README.md)를 따른다. 읽기 전용 준비 도구는 정제를 재개하지 않는다. 03의 재생성 절차는 아래 접힌 규칙의 초기화와 재정제, 04의 검토 계약은 [에이전트와 함께 검토하기](#에이전트와-함께-검토하기)에 있다.
...
검토 흐름은 위 그림의 04다. 에이전트 · Skill → CLI 개념 비교 → 사용자 확인 → CLI 검토 확정 순서이며, Version 3은 검토 완료인 Version 1과 비교하고 검토 이력이 없으면 직전 Version과 비교한다.
