...
모델이 JSON 필드 내부의 줄 번호를 셀 수 있으므로, 인용문이 **현재 청크의 디코딩된 본문에 정확히 한 번 일치**하면 서버가 불변 L1 행과 전체 원문 인용으로 연결한다. 예: 보관 행 159 내부의 본문 10번째 줄도 근거 위치는 L1 행 159다. 다른 원문·개정, 중복 일치, 공백/대소문자가 다른 유사 인용, 참고 맥락만의 일치는 보정하지 않는다. 연결 뒤에도 범위·생략 구간·발화자·정정 권한·원문 일치 검사를 적용한다. L1과 원래 모델 응답은 보존하고 연결 횟수는 `anchoredEvidence`로 기록한다.
