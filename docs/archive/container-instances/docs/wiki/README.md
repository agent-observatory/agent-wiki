...
아래 그림은 OCI App CI와 PostgreSQL 전용 Compute VM을 사용하는 설계안이다. Caddy 인증서는 비공개 Object Storage에 저장한다. CI 무료 계정 자격과 실제 저장 모듈 연동은 아직 검증하지 않았으며 구축 전이다. 지식 모델·검색 예제·자원 배분·운영 기준은 아키텍처에서 관리한다.

![원천 자료부터 근거 있는 응답까지의 L1–L5](../assets/wiki-layers.svg)

![OCI App CI와 사설 연결된 PostgreSQL 전용 VM](../assets/wiki-deployment.svg)
