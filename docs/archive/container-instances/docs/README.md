...
**설계 단계. 앱 구현·자원 생성·배포는 아직 하지 않았다.**
...
- 앱은 OCI Container Instances, DB는 별도 Compute VM의 단일 PostgreSQL이다. 후보 배분은 앱 3 OCPU·20GB + DB 1 OCPU·4GB다. Caddy 인증서 상태는 별도 비공개 Object Storage 버킷에 둔다. 무료 범위만 허용하며 CI 계정 자격과 S3 모듈 연동은 구축 전에 확인한다.
