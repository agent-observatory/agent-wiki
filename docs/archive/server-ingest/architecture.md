...
![A1 VM 1대에서 앱과 PostgreSQL을 함께 실행](wiki-deployment.svg)

이전 App Container Instance + DB VM 분리안은 [그림·생성기와 함께 보존](../container-instances/ARCHIVE.md)했다. 현재는 단일 VM이며 필요해지면 앱·Worker·DB를 분리한다.
...
![외부 빌드 후 변경된 앱만 교체하고 DB와 인증서를 유지](wiki-operations.svg)
