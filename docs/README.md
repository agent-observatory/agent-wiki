...
**개인 에이전트 정제·Wiki 저장/조회 구조와 메뉴별 페이지를 구현했다. 로컬·CI 검증과 OCI 전환, 실제 원문·지식 등록을 완료했다.** 기능별 검증 상태는 [운영 현황](OPERATIONS.md), 다음 우선순위는 [첫 배포 다음 작업](wiki/architecture.md#첫-배포-다음-작업)을 따른다.
