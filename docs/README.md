...
세부 결정과 변경 기준은 [아키텍처](wiki/architecture.md)를 따른다. 현재는 개발 모드이며 Wiki 데이터·API·키를 초기화하고 하위 호환성 없이 재구성할 수 있다. 사용자가 운영 모드라고 선언하면 보존 정책을 다시 정한다. 이번 문서 정비에서 데이터·토큰·런타임은 바꾸지 않는다. 프로젝트 역사와 리니지는 원격 Wiki에 보관한다.
