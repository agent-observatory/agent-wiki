...
- 선택 메뉴는 Workspace·문서 종류·권한에 같은 테마 컴포넌트를 사용한다. 선택 표시·키보드 이동·포커스 복귀는 [Radix Select](https://www.radix-ui.com/primitives/docs/components/select)를 사용하고 색상·간격은 앱 토큰으로 지정한다.
- [OpenGateway Docs](https://opengateway.ai/docs)의 중성 검정·회색 표면, 얇은 경계선, 파란 링크를 참고한다(2026-09-12 화면 확인). 청록색 포인트와 남색 표면은 사용하지 않는다. 라이트는 흰 바탕·중성 회색에 같은 파란 포인트를 적용한다.
- 문서 목록은 구분선으로 나누고 본문은 페이지 바탕에 이어지는 평면으로 둔다. 선택 행은 얇은 파란 왼쪽 선으로 표시한다. 입력·메뉴는 6–10px 반경과 1px 경계선을 사용한다.
- 주 행동은 파란 글자·테두리와 옅은 배경, 보조 행동은 중성 테두리로 표현한다. 제목·브랜드·일반 메타데이터는 중성색으로 유지한다. 화면을 변경할 때 관련 토큰과 이 문서도 함께 갱신한다.

## 색상 역할

| 역할    | 다크      | 라이트    | 사용처               |
| ------- | --------- | --------- | -------------------- |
| Canvas  | `#0C0C0E` | `#FFFFFF` | 페이지 배경          |
| Surface | `#121215` | `#FFFFFF` | 패널·사이드바·입력   |
| Raised  | `#202025` | `#F1F1F4` | 태그·코드·호버       |
| Text    | `#EDEDEF` | `#202024` | 본문·제목            |
| Muted   | `#A0A0AA` | `#62626B` | 보조 설명·메타데이터 |
| Accent  | `#58A6FF` | `#0969DA` | 주 행동·링크·선택    |
| Soft    | `#151F2D` | `#EDF5FF` | 선택·근거 배경  |
| Warning | `#F0CA7E` | `#785A16` | 진행·재시도          |
| Error   | `#FFB4BF` | `#983E31` | 오류·최종 실패       |

웹의 실제 토큰은 `apps/web/app/style.css`에서 관리한다. 주 행동의 글자는 다크·라이트 모두 해당 테마의 파란 Accent를 사용한다.
