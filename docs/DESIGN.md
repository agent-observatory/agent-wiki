...
- 반영 이력은 평면 목록과 가로 문서 링크로 묶고, 원문 목록은 좁은 화면에서 메타데이터를 다음 줄로 내린다. 짧은 날짜에는 연도·월·일·24시간 시각을 표시하고 전체 시각을 title에 보존한다.

- 호출 이력은 상태 배지·모델·실행 지표·시각을 한 줄 중심으로 표시한다. 공통 StatusBadge로 성공·완료는 초록, 실패는 빨강, 진행 중은 파랑, 중단은 주황으로 표시하고 문구를 함께 둔다. 세션·업로드·일별 이력에도 같은 색상을 쓴다. 오류·당시 재시도 정보만 다음 줄에 묶고 좁은 화면에서는 자연스럽게 줄바꿈한다.
...
컴포넌트 간에는 최소 32px, 그룹 경계 안쪽에는 최소 32px 여백을 둔다. 카드 높이는 내용에 맞추고 그룹 제목과 내용을 분리한다. Caddy → API → PostgreSQL은 같은 높이의 직선, Web → API는 세로선으로 둔다. 볼륨과 원문 저장소는 연결선이 다른 컴포넌트를 관통하지 않는 위치에 배치한다. Caddy 영속 상태는 같은 연결 볼륨의 별도 경로임을 표시한다. DuckDNS 도메인은 웹에 표시하고 VM IP로 연결됨을 명시한다. DNS·인증서 발급 기관을 HTTP 요청이 통과하는 중계 서버처럼 그리지 않는다. 작업 에이전트의 조회와 Collector의 수집 선을 분리한다. Collector → Caddy/API는 위치 확인·업로드 허가·완료 통지, Collector → Object Storage는 압축 증분 본문의 직접 전송으로 구분한다.
