...
그림은 두 장만 둔다. **아키텍처 한 장(`wiki-architecture.svg`)과 L1–L5 흐름 한 장(`wiki-l1-l5.svg`)**이다. 관계·흐름이 아닌 사실 목록(포트·자원 한도·변경 의도의 경우·실험 순서·재생성 단계·운영 경로·Retry/Reprocess/Reassemble 비교)은 그림에 넣지 않고 문서의 표로 둔다. 새 그림을 추가하는 대신 두 장 중 하나를 고친다.
...
- `wiki-l1-l5.svg`: 아키텍처 문서와 L2·L3 기억 설계 문서가 같은 파일을 삽입한다. 번호를 붙인 네 띠를 위에서 아래로 쌓는다. 01은 두 세션의 L1 증분 → Worker의 청킹·입력 조립·AI 판단·서버 검증이다. L3 현재 주장의 BM25 후보·규칙 재정렬은 조회 색으로 그리고 선택한 후보는 AI 카드가 아닌 입력 조립으로 되돌린다. 검증 실패가 같은 청크를 AI로 다시 보내는 루프는 정제 색의 점선이며 옆의 Retry 카드가 설명한다. 서버 검증 → L3 반영과 L3 → 읽기 카드의 참고 조회는 서로 다른 x에 둔다. 02는 L3 그룹 안의 Decision A·B·C와 Wiki Page 두 장이다. Decision에서 페이지로 내려가는 선은 내용의 출처를 따른다. 현재 상태는 B·C에서, Decision History는 A와 B → A supersedes에서 내려오므로 B는 두 페이지로 모두 이어지고 대체된 A도 페이지와 끊기지 않는다. 03은 L5 에이전트 카드 ↔ L4 그룹(후보 → Claim·관계 → 원문 구간)이며 Claim 카드는 위의 L3 그룹에서, 원문 카드는 작은 L1 카드에서 읽는다. 에이전트가 원문 저장소를 직접 읽는 선은 그리지 않는다. 조회 이력은 기록 색 점선으로 잇는다. 04는 Skill → CLI 비교 → 사용자 확인 → CLI 확정과 Version 1·2·3이다. Version 3 → Version 1의 비교 기준은 기록 색 점선이다. 처리 단계는 논리 역할이며 별도 배포 앱으로 오인시키지 않는다. 현재 채택과 사실 검증, 발언·실제 적용·수집 시각을 구분한다. 상위 설명은 짧게 두고 구현 규칙은 문서의 접힌 영역에 둔다.
