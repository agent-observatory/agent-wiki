...
레퍼런스의 흰 배경 금지·전용 글꼴(Instrument Serif·Geist)·단일 코랄 강조는 따르지 않는다. 우리는 흰 캔버스, Noto Sans KR, 흐름별 의미 색 4종으로 이미 정해져 있다.
...
화살표는 원문·지식 반영 `#B35C00`, 조회·응답 `#245CC5`, 인증·운영·기록·기준 `#68778B`, 대체 관계 `#8054A3`로 구분한다. 같은 색의 점선은 그 흐름의 보조 경로(정제 색 점선 = 재시도, 회색 점선 = 설정·인증·기록·비교 기준)다. 범례는 제목 옆에 둔다. 반복하는 회색 부제는 넣지 않는다. 요청·응답은 양방향, 저장·반영은 단방향이며 긴 왕복 경로 대신 나란한 직교선을 쓴다.
