...
- `wiki-knowledge-model.svg`: L1–L5와 L3 내부의 Claims·Decisions·Wiki Page를 한 그림에 둔다. 계층 상자는 진한 남색 상단 제목 띠·흰 제목과 파스텔 본문으로 통일한다. Decision 카드에서 Wiki Page로 내려가는 선은 내용의 출처를 따른다. 현재 상태와 설명은 현재 결정 B·C에서, Decision History는 이전 결정 A와 B → A supersedes 관계에서 내려오므로 B는 두 페이지 상자로 모두 이어진다. 대체된 결정도 페이지와 끊기지 않는다. supersedes 라벨은 두 카드 사이 화살표에 붙이고 범례는 제목 옆에 둔다. 요청·응답인 L4 ↔ L5는 양방향이다. 변경 예시는 합성임을 밝힌다.
...
- `wiki-l2-l3-memory.svg`: L2·L3 기억 설계 문서의 그림 한 장이다. 아키텍처 문서 밖의 그림은 이 한 장에 번호를 붙여 위에서 아래로 쌓고 절 사이에는 옅은 가로선을 둔다. 01 증분 정제와 결정 이력은 두 세션의 L1 증분 → Worker의 청킹·입력 조립·AI 판단·서버 검증 → 변경 의도 판단 → L3 주장 A/B와 supersedes → L4의 세 질문이다. L3 현재 주장의 BM25 후보·규칙 재정렬은 참고 조회 색으로 그리고 선택한 후보는 AI 카드가 아닌 입력 조립으로 되돌린다. 02 방법을 고르는 실험, 03 원문을 유지하는 재생성, 04 에이전트 검토가 이어진다. 처리 단계는 논리 역할이며 별도 배포 앱으로 오인시키지 않는다. 현재 채택과 사실 검증, 발언·실제 적용·수집 시각을 구분한다. 상위 설명은 짧게 두고 구현 규칙은 문서의 접힌 영역에 둔다.
