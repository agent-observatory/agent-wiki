...
- `wiki-curation.svg`·`wiki-decision-history.svg`·`wiki-curation-evaluation.svg`·`wiki-curation-rebuild.svg`: 새 증분·세션 맥락·관련 지식의 입력, 원자적 반영, 근거 있는 주장 대체, 청킹/리랭킹 비교 실험, L1을 보존하는 재생성을 각각 한 그림으로 설명한다. 처리 단계는 논리 역할이며 별도 배포 앱으로 오인시키지 않는다. 상위 설명은 짧게 두고 구현 규칙은 Curation 문서의 접힌 영역에 둔다.
