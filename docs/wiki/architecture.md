...
첫 실사용 자료는 [Atlas → Sessions → 독립 Wiki의 역사와 결정 5개](project-history.md)다. 2026-09-12 개인 작업 Workspace에 검토한 Article을 직접 등록했다. NVIDIA 추출 성공과 별개로 조회 흐름을 확인한다. 대표 질문은 계보·검색 선택·인프라 결정·실제 완료 상태·근거 없음으로 구성하고, 정정 후 재조회도 확인한다. 접근 키는 로컬 비밀 설정에 두고, Workspace 경계를 유지한다.
