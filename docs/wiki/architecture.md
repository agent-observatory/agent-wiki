...
**현재 배포:** 원문·지식·근거 저장, 키워드 조회, CLI와 웹. **미구현 설계:** Collector·Watcher와 자동 군집화·백그라운드 정제. 아래 그림은 새 설계이며 완료 상태는 [운영 현황](../OPERATIONS.md)을 따른다. [수집·정제 계약과 전환 순서](agent-memory.md).
...
| Collector·Watcher | 허용한 세션 기록을 읽기만 하고 변경분 보관 | 미구현. 클라이언트별 어댑터·수집 계약 필요 |
