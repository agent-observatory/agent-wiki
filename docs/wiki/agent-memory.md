...
Worker는 PostgreSQL 작업 테이블에서 하나씩 실행한다. `429`·일시적인 서버 오류는 제공자 대기 안내와 backoff를 적용해 최대 3회 시도한다. 인증 오류·틀린 인용·개정 충돌은 같은 요청을 자동 반복하지 않는다. 입력은 호출 전에 예산에 맞춰 나눈다. 제공자 한도 초과·출력 잘림은 완료로 처리하지 않고 오류를 남겨 한도 조정 후 재시도한다. 작은 입력에서도 시간 초과가 반복되면 설정을 중지하고 연결·모델 상태를 확인한다.

NVIDIA `202`는 요청 ID로 기존 결과부터 조회한다. 모델 출력 저장 뒤 종료됐다면 저장한 출력으로 멱등 반영을 복구한다. 지식 반영·작업 완료는 같은 DB 트랜잭션이다. 요청 ID 없이 응답이 유실되면 처리 여부가 불명확하며 재호출로 사용량이 중복될 수 있다. 응답 없는 호출의 토큰은 미집계로 남기고 UTC 자정에 일일 한도를 갱신한다.
