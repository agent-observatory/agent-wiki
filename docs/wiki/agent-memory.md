...
| 계층 | 보존하는 것 |
| --- | --- |
| L1 · Raw Sources | 고정 보관본·해시·기록 위치. 추가 기록은 새 원문 |
| L2 · Curation | 입력 원문·참고 지식 개정·제공자·모델·설정·프롬프트 버전·모델 출력·사용량·오류 |
| L3 · Knowledge | 지식 개정·주장·원문 인용·대체 관계·반영 주체 |
| L4 · Query | 선택한 지식 개정과 인용 자료 |
| L5 · Answers | 에이전트 답변·작업. 실제 활용 기록이 없으면 추적 완료라고 하지 않음 |
