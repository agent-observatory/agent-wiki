...
**사용자는 Claude·Codex로 하던 작업을 계속한다.** Collector는 기록 파일을 읽고, 원격 Worker가 외부 AI API로 정제한다. 수집 명령·정제 요청·대기 시간을 작업 대화에 끼워 넣지 않는다. [전체 구조](architecture.md) · [정제 알고리즘](curation.md) · [실제 배포·검증](../OPERATIONS.md).
...
주장과 근거·정정 관계의 통합 기준은 [Curation](curation.md#주장과-관계)을 따른다. 사용자 결정·관찰·AI 해석·미확인을 구분한다.
...
| `GET /refinement-sessions/:id/jobs` | 선택 세션의 개별 작업. `detailPage`·`pageSize` |
