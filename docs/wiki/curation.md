# L2 · Curation — 증분에서 기억으로

[전체 아키텍처](architecture.md) · [설치·수집·API](agent-memory.md) · [구현·검증 상태](../OPERATIONS.md#증분-맥락과-결정-통합-설계)
...
![Supabase 결정의 취소와 OCI PostgreSQL 결정의 근거·대체 관계](../assets/wiki-decision-history.svg)
...
관계는 **주장 Version 사이**에 저장한다. 관계의 방향·종류·근거·생성 실행을 보존하고 자기 참조·대체 순환을 거부한다. 원래 발언 시각과 수집·반영 시각을 분리한다. 하나의 관계로 페이지 전체를 폐기하거나 언급되지 않은 기존 주장을 지우지 않는다. 새 개정에는 변경하지 않은 주장과 기존 인용을 유지한다.
...
기본 목록은 **세션 한 줄 + 대기·진행 중·실패·완료·청크 반영**이다. 서버에서 세션별 집계 후 페이지를 나누며 작업 수와 세션 수를 구분한다. 재시도 대기는 대기에 포함해 보조 표시한다. 펼친 세션의 작업·오류·재시도·반영 결과만 별도 페이지로 읽는다. 작업 상태와 청크 진척도는 별개이고 미분할 자료가 있으면 완료율로 숨기지 않는다.
...
기본 DeepSeek Flash, 동시성 1·같은 호스트/키당 최대 20 RPM(폴링 포함)을 유지한다. 일시 오류는 최소 2분 대기·지수 증가·무작위 지연과 Retry-After를 적용한다. 인증·입력·근거 오류는 확인 필요 상태로 남긴다. 종료·재시도 이력 계약은 [실행 계약](agent-memory.md#재시도와-종료)을 따른다.
