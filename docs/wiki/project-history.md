...
| 지식 정제 | 개인 에이전트가 정제하고 Wiki에 반영하는 목표 | 서버 NVIDIA Worker | 사용 중인 에이전트의 모델·사용량으로 처리. 서버 경로는 아직 교체 전 |
...
초기 등록했던 다섯 주제를 새 원문·정제 계약으로 다시 만든다. 기존 요약과 링크를 원문 증거로 간주하지 않고 비밀을 제외한 실제 발췌를 등록한다. 기존 지식·개정·ID 보존은 요구하지 않는다.
