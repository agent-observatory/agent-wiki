...
실제 정제 입력·출력 상한 16,384 비교를 진행한다. [측정값](../experiments/curation/provider-diagnosis-results.json)에는 원문·키·모델 출력 내용을 저장하지 않는다. 모델 비교는 L3에 반영하지 않으며, 응답 성공과 정제 품질 검증을 구분한다.
