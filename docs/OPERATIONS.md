...
실제 토큰이 추정의 30%이므로 DeepSeek에도 Qwen처럼 `o200k_base` 추정(10% 여유)을 적용하면 같은 30K 목표에서 청크 수가 약 1/3로 줄어 호출 수와 지침 반복 비용이 감소한다. 단 한 호출의 원문이 3배 길어지므로 12개 변경 상한이 더 자주 닿고 긴 입력의 추출 누락 가능성이 있다. 이번 변경의 효과를 먼저 측정한 뒤 별도로 비교한다. 참고 지식의 근거 추가만으로 Version이 오를 때 `CURATION_CONTEXT_CHANGED` 재생성이 늘어날 수 있는데, 현재 운영 이력에는 0건이라 설계(모든 Version 변경은 맥락 재비교)를 유지한다.
