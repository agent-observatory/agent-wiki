...
2026-09-13. 사용자 요청으로 모델 호출 제한을 150 → 330초(5분 30초), 작업 임대를 5 → 7분으로 조정했다. 실행 진단에 당시 제한과 임대 시간을 기록한다. NVIDIA 비동기 결과 조회도 40회에서 조기 종료하지 않고 호출 기한 안에서 같은 요청을 조회한다. 호출 간격·재시도 대기·종료 시 90초 유예는 유지한다. 관련 회귀 검사와 타입 검사를 통과했다. 실제 배포는 확인 후 기록한다.

마지막 시간 초과 요청의 메시지 JSON은 5,127바이트였고, 원문 텍스트 1,434바이트·시스템 지침 2,668바이트였다. 출력 상한은 2,048토큰이다. 100MB 원문 전체 전송은 아니며 제공자 지연과 요청 옵션을 별도로 검증한다. NVIDIA DeepSeek V4 추론 옵션은 [공식 예제](https://build.nvidia.com/deepseek-ai/deepseek-v4-flash-0731)의 `chat_template_kwargs` 형식으로 맞췄다. 로컬 요청 본문 검사로 차이를 재현했으며 이 차이가 시간 초과의 원인인지는 실제 비교 결과와 구분한다.
