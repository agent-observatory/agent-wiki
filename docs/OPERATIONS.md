...
- 자동 정제 재개는 승인됐으며 첫 실행의 실패를 수정 중이다. Codex는 Agent Wiki 프로젝트만 10분마다 수집하며 Claude는 전체 비활성이다.
...
구현과 실제 운영 적용을 구분한다. 현재 수정은 배포 전이며, 배포 후 설정 변경·실패 작업 재시도·재개·첫 반영 여부를 아래에 추가한다. 합성 Worker·청킹·세션 간 동일 주장/동시 갱신/리니지 회귀를 포함한 전체 검사 172개·타입 검사·빌드와 실제 원문 읽기 전용 재생을 통과했다.
