...
2026-09-13. 반복 실패 때 대기가 2·4·8·16·32분으로 늘어나던 정책을 **매번 120~144초**로 변경했다. 더 긴 제공자 `Retry-After`만 예외로 지키며, 오류 횟수·원인·청크 진행 위치는 유지한다. 연속 다섯 번 실패해도 대기가 늘지 않는 Worker·공유 키 검사 13개와 타입 검사를 통과했다. 배포와 기존 대기 시각 조정은 이어서 확인한다.
