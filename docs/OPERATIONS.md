...
2026-09-13. “왜?”·“어디에”·“???”를 정리하면 검색어가 없어져 전체 지식이 반환되는 문제를 재현했다. 빈 검색창의 목록 조회와 구분해 항목·인용을 비우고 `queryStatus: needs_terms`로 핵심어 추가를 안내한다. 웹에서도 자료 등록 안내 대신 검색어 안내를 보여준다. 검색할 말이 없다는 것과 지식이 없다는 것을 혼동하지 않는다. 전체 검사 111개·타입 검사·데스크톱/390px 모바일 안내·검색어 제거 후 목록 복귀·그림 XML/렌더링을 확인했다. 배포는 다음 확인 단계다.
