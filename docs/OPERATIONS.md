...
2026-09-13. Collector 기본은 전체 프로젝트이며 `--project` 또는 `projects` 배열로 범위를 제한한다. 현재 설치 설정은 Agent Wiki 프로젝트 하나로 유지한다. 자동 정제는 최대 20 RPM·동시 실행 1개를 유지하며 일일 제한 기본값을 없음으로 바꾸고 필요할 때만 직접 설정한다. 상태 카드·설정 화면·그림을 같은 기준으로 정리했다.
