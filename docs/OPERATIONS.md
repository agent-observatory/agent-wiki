...
실제 실패 응답 재검증에서 해당 인용이 2/3 → 3/3 일치했으며, 그 입력 자체도 운영 지침으로 정상 제외됐다. `Lint`를 `Lineage`로 바꾼 다른 실제 오인용은 계속 거부됐다. 기존 반영 인용 14개에는 새 제외 범위가 없었다. 조각 경계·누락·다른 이벤트/필드·잘못된 단어·중첩 역할·Worker 반영 회귀 검사를 포함해 전체 114개와 타입 검사를 통과했다. 배포 결과는 아래에 기록한다.
