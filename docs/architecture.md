...
![L1부터 L3까지 한 장. 01 세션 증분을 Worker가 청킹·입력 조립·AI 추출·서버 검증·반영으로 정제한다. 입력 조립은 세션 메타데이터·암호화 본문·지침과 함께 Wiki CLI 호출과 그 출력 블록을 비워 위키가 자기 출력을 다시 먹는 순환을 끊는다. L3 현재 주장의 BM25 후보를 입력 조립에 되돌리고, 출력 오류는 같은 청크를 다시 보내며, 어긋난 관계는 그 관계만 버리고 나중에 유효해질 수 있는 것만 통합 대기함에 넘긴 뒤 주장은 그대로 반영한다. 02 통합 전 L3에는 같은 subject·scope에 current 주장 A·B가 함께 남고 제안 D와 대기함의 관계가 있다. 03 Consolidation Job이 gather → model → validate → publish 네 Step으로 관계만 판단해 자동 반영하며, gather는 묶음 지문이 직전 완료 Job과 같고 대기함이 비었으면 모델을 호출하지 않고 Step별 상태를 따로 기록한다. 04 Wiki Page 새 Version은 현재 주장만 나열하고 supports로 이어진 묶음은 코드가 고른 대표 하나로 접어 한 번만 실으며 클릭한 주장의 리니지 패널이 B가 A를 대체한 관계와 이유를 보여준다. 검토의 relation reject는 정정 Version을 발행하고 거절을 기억한다](assets/wiki-l1-l3-curation.svg)
