...
**사용자의 작업과 기록의 수집·정제를 분리한다. 수정·검토·설정은 CLI, 웹은 뷰어이며 자동 정제 중지·재개도 제공한다.** Claude·Codex는 사용자와 작업하고 필요할 때 Wiki를 조회한다. 단일 설치하는 Agent Wiki Client의 백그라운드 Collector가 클라이언트 기록을 읽어 원격에 보관하고, 원격 Worker가 텍스트를 청킹해 정제한다.
...
![L1–L5의 정제·결정 변경·원문 근거 추적과 독립적인 조회](assets/wiki-layers.svg)

그림의 Compose → K3s는 합성 예시다. **지식은 현재 결정과 변경 이유를 함께 보존하고, 원문 근거까지 역추적한다.**
