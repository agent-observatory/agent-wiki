...
L1 → L3 정제는 아래 한 장이다. 위에서 아래로 **01 L1 증분 → L2 청크 추출(청킹 → 입력 조립 → AI 추출 → 서버 검증 → 반영, L3 현재 주장의 BM25 후보 되돌림, 출력 오류의 같은 청크 재시도, 관계만 실패한 반영의 대기함 지연), 02 추출이 L3에 남기는 통합 전 주장(같은 subject·scope에 current 둘, 제안, 대기함의 관계), 03 주제마다 관계만 판단하는 Consolidation Job의 gather → model → validate → publish, 04 그 결과인 Wiki Page 새 Version의 현재 주장 목록·클릭한 주장의 리니지 패널·검토와 relation reject**다. 03·04는 구현·배포를 마쳤고 그림도 실선으로 갱신했다. 실선은 구현·배포를 뜻하며 운영 검증을 뜻하지 않는다. 실제 자료로의 검증 상태는 `OPERATIONS.md`를 따른다. 같은 그림을 [L2·L3 기억 설계](l2-l3-memory.md)가 자세히 설명한다.

![L1부터 L3까지 한 장. 01 세션 증분을 Worker가 청킹·입력 조립·AI 추출·서버 검증·반영으로 정제한다. 입력 조립은 세션 메타데이터·암호화 본문·지침과 함께 Wiki CLI 호출과 그 출력 블록을 비워 위키가 자기 출력을 다시 먹는 순환을 끊는다. L3 현재 주장의 BM25 후보를 입력 조립에 되돌리고, 출력 오류는 같은 청크를 다시 보내며, 어긋난 관계는 그 관계만 떼어 통합 대기함에 넘긴 뒤 주장은 그대로 반영한다. 대기함은 두 상태다. 나중에 유효해질 수 있는 pending은 다음 통합이 모델에 다시 보내고, 게이트가 절대 받을 수 없는 needs_human은 모델로 돌아가지 않고 review conflicts와 웹 Curation 탭에서 사람이 판단한다. 02 통합 전 L3에는 정본 subject·scope에 current 주장 A·B가 함께 남고 제안 D와 대기함의 관계가 있다. B의 slug는 갈라져 있었고 사람이 subject alias로 이어 관계 게이트·통합 묶음 키·어휘 힌트·페이지 조립이 정본 이름으로 비교한다. 후보 계산은 모델 없이 하고 주장에 저장된 slug는 바꾸지 않는다. 03 Consolidation Job이 gather → model → validate → publish 네 Step으로 관계만 판단해 자동 반영하며, gather는 묶음 지문이 직전 완료 Job과 같고 대기함이 비었으면 모델을 호출하지 않고, model은 요청 크기를 재어 예산 안에 드는 묶음까지만 보내고 남은 묶음은 미판단으로 남겨 후속 Job이 이어받는다. Step별 상태는 따로 기록한다. 04 Wiki Page 새 Version은 현재 주장만 나열하고 supports로 이어진 묶음은 코드가 고른 대표 하나로 접어 한 번만 실으며 클릭한 주장의 리니지 패널이 B가 A를 대체한 관계와 이유를 보여준다. contradicts는 양끝이 모두 current일 때만 conflicted를 만든다. 검토의 review conflicts는 needs_human 관계를 사람에게 보여 별칭·관계·철회 중 하나를 고르게 하고, relation reject는 정정 Version을 발행하고 거절을 기억한다](assets/wiki-l1-l3-curation.svg)

그림의 NVIDIA → Alibaba는 합성 예시다. **지식은 현재 결정과 변경 이유를 함께 보존하고, 원문 근거까지 역추적한다.** 추출은 청크마다 주장을 만들고, 통합은 주제마다 그 주장들 사이의 현재·이력 관계를 정한다. 리니지 패널은 그 관계를 사람이 읽는 방법이며, 통합이 끝나지 않은 주제는 **통합 대기**로 표시해 완성된 페이지로 오해하지 않게 한다. 통합·리니지 패널·relation reject의 규칙은 [통합 · Consolidation](l2-l3-memory.md#통합--consolidation--구현-완료--운영-검증-전)에 있고 구현·배포는 마쳤으며 실제 세션 자료로의 운영 검증은 아직이다. 같은 대상·범위에서 변경 의도를 어떻게 가르는지(명시적인 변경·검토 의견·다른 적용 범위·불명확한 결론·늦게 수집된 발언)는 그림이 아니라 [주장과 관계 표](l2-l3-memory.md#주장과-관계)에 둔다.
...
| 값 | 묻는 것 | 규칙 |
| --- | --- | --- |
| `subject` | 무엇에 대한 주장인가 | 구체적인 대상 하나의 소문자 slug(`k3s`, `tls-certificate`). 주제 키와 같을 수 없고, 모델은 그 주제가 이미 쓰는 목록에서 고른다. 갈라진 slug는 사람이 별칭으로 잇는다 |
| `scope` | 어디에 적용되는가 | 닫힌 다섯 값: `general` · `local` · `production` · `dev-mode` · `experiment` |

이 구조를 [어노테이션·시맨틱 레이어·온톨로지](README.md#어노테이션시맨틱-레이어온톨로지의-구분) 세 층으로 읽으면 이렇다. 주장의 본문과 근거는 낱개에 붙는 **라벨**이다. `type`·`scope`의 닫힌 목록과 관계 게이트·상태 계산식은 **한 곳에서 한 번 정의해 API·Worker·검토·페이지가 같은 답을 내게 하는 정의 층**이다. 관계로부터 저장되지 않은 `superseded`·`conflicted`와 대표 주장을 도출하는 것이 우리가 가진 **유일한 추론**이며, 관계의 모양에서만 도출하고 시각·유사도·속성값 규칙으로는 도출하지 않는다. `subject`의 정의 층은 **별칭 하나뿐**이다. 어휘 자체는 정의하지 않고 주제가 이미 쓰는 목록을 사용 빈도순으로 모델에 힌트로 준다. 한 속성이 두 slug로 갈라지면 사람이 `subject alias`로 둘을 잇고, 그때부터 관계 게이트·통합 묶음·어휘 힌트·페이지 조립이 정본 이름으로 비교한다. **주장에 저장된 slug는 바꾸지 않는다** — 그건 모델이 그때 말한 것이고 불변 Revision의 일부다. 별칭은 지식이 아니라 Workspace 설정이라 발행 없이 넣고 뺄 수 있고 재구성이 지우지 않는다. 사슬(A→B, B→C)은 거부한다.
...
**기존 원문과 지식을 유지하고 필요한 분석·조립만 다시 한다.** 분석 오류를 고친 개정과 사용자가 A에서 B로 결정을 바꾼 이력은 구분한다. 세 가지 재작업은 L1 → L3 그림의 서로 다른 위치로 다시 들어간다. Retry는 01의 서버 검증 → AI 재시도 루프, Reprocess의 후보는 04의 검토 카드, Reassemble은 04의 Wiki Page 새 Version이다. Consolidation은 03이며 기존 주장 사이의 관계만 다시 판단한다.
...
원본 링크와 사용자 메모는 [문서 안내의 레퍼런스](README.md#레퍼런스)에 보존한다. NAVER의 OpenMetadata 활용은 사용자 제공 메모이며, 위 표는 공식 발표 소개에서 확인한 범위다. GeekNews 글은 Karpathy 원문을 소개하는 자료로 함께 참고한다.

원본 링크·확인 시점·사용자 메모는 [문서 안내의 레퍼런스](README.md#레퍼런스)에 모아 둔다.
