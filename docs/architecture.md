...
이 구조를 [어노테이션·시맨틱 레이어·온톨로지](README.md#어노테이션시맨틱-레이어온톨로지의-구분) 세 층으로 읽으면 이렇다. 주장의 본문과 근거는 낱개에 붙는 **라벨**이다. `type`·`scope`의 닫힌 목록과 관계 게이트·상태 계산식은 **한 곳에서 한 번 정의해 API·Worker·검토·페이지가 같은 답을 내게 하는 정의 층**이다. 관계로부터 저장되지 않은 `superseded`·`conflicted`와 대표 주장을 도출하는 것이 우리가 가진 **유일한 추론**이며, 관계의 모양에서만 도출하고 시각·유사도·속성값 규칙으로는 도출하지 않는다. `subject`는 아직 정의 층이 아니라 라벨이다 — 어휘를 한 곳에 정의하지 않고 주제가 이미 쓰는 목록을 사용 빈도순으로 모델에 힌트로 줄 뿐이다.
