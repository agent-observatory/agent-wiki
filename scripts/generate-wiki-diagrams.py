...
canvas(1560,1050,'결정 변경은 덮어쓰기 대신 관계로','합성 예시. 서로 다른 증분의 Supabase 결정과 취소·OCI PostgreSQL 결정은 같은 대상의 주장이다. 각 원문과 주장을 보존하고 새 주장에서 이전 주장으로 근거 있는 대체 관계를 연결한다. 불명확한 충돌은 두 주장을 보존한다. 관계는 기존 PostgreSQL에 저장한다.')
legend(1070,48,'정제·반영','ingest');legend(1300,48,'근거·관계','relation')
box(40,170,1480,48,'#344256','#344256')
text(60,203,'Workspace · 개인 작업     /     대상 · Agent Wiki 운영 DB',FONT['group'],True,'#FFFFFF')
card(40,260,550,160,'이전 증분 · '+layer_label(1),['사용자: “DB는 Supabase로 하자.”','원문 A · 발언 시각 · 고정 구간'],'data','tabler-book-2')
card(970,260,550,160,'새 증분 · '+layer_label(1),['사용자: “Supabase는 취소하고','OCI에 PostgreSQL을 설치하자.”'],'data','tabler-book-2')
path('M315 420 V510',flow='ingest');path('M1245 420 V510',flow='ingest')
card(40,510,550,190,'이전 주장 · 보존',['운영 DB = Supabase','사용자 결정 · 과거 선택','근거: 원문 A'],'ops','tabler-clipboard-check')
card(970,510,550,190,'새 주장 · 현재 선택',['운영 DB = OCI PostgreSQL','사용자 결정 · 명시적인 변경','근거: 새 증분의 정정 발언'],'data','tabler-clipboard-check')
path('M970 590 H590',flow='relation')
text(664,564,'대체 · supersedes',FONT['component'],True,FLOW_COLORS['relation'])
text(639,624,'같은 대상·범위 + 정정 근거',FONT['label'])
text(644,657,'도착 순서만으로 대체하지 않음',FONT['label'])
group(40,790,1480,190)
text(64,826,'agent-wiki-db · 주장 Version과 근거를 연결',FONT['group'],True,'#FFFFFF')
card(64,860,442,96,'명시적인 취소·대체',['철회·대체 관계 + 변경 근거'],'data','tabler-clipboard-check')
card(530,860,442,96,'해석이 불분명한 충돌',['두 주장 보존 · 판단 보류'],'ai','tabler-clipboard-check')
card(996,860,500,96,'검색 → 재정렬 → 관계 확인',['현재·과거·충돌과 근거를 함께 반환'],'app','tabler-book-2')
