...
card(696,468,288,188,'입력 조립',['새 청크 · 선별한 텍스트','메타데이터·지침 제외','원문 위치 · 참고 맥락'],'ingest','tabler-book-2')
card(1024,468,288,188,'주장 추출 · 비교',['AI Provider · 텍스트 호출','추가 · 정정 · 취소','불명확하면 보강 요청'],'ai','openai')
card(1352,468,268,188,'근거 연결·검증',['유일한 인용 → 원문 행','역할 · Version · 정정 권한','불확실 → 미확인'],'ingest','tabler-clipboard-check')
...
canvas(1560,510,'L4 · Query · 검색어에서 근거까지','조사를 포함한 원래 검색어와 제한적인 대안을 함께 찾는다. 여러 검색어 일치와 필드 가중치로 후보를 정렬하고 현재 상태·범위·대체 관계를 확인한 뒤 근거를 반환한다. 일반 조회에는 AI 호출이 없다.')
card(40,135,335,190,'검색어 준비',['원래 표현 + 조사 제거형 검색','중복 검색어는 한 번 계산','핵심어 없으면 검색어 안내'],'app','tabler-terminal-2')
card(420,135,335,190,'후보 정렬',['일치한 검색어 수 우선','제목 4 · 태그/별칭 3 · 본문 1','페이지 분할 전에 정렬'],'app','tabler-book-2')
card(800,135,335,190,'유효성 확인',['현재 / 과거 · 적용 범위','대체 관계 → 현재 주장','미해결 충돌은 함께 반환'],'data','tabler-clipboard-check')
card(1180,135,340,190,'Context 반환',['검색어를 함께 담는 구간','고정 Version · 원문 근거','최대 6개 · 반환 예산 적용'],'app','tabler-book-2')
for x in [375,755,1135]:path(f'M{x} 230 H{x+45}',flow='query')
text(40,395,'“임베딩 비용” → 두 검색어를 담은 결정 문서 → “임베딩”만 제목에 있는 참고 문서보다 우선',FONT['body'])
text(40,445,'일반 조회 · AI 호출 없음     |     관련도는 사실 여부가 아님     |     “미뤘지 ↔ 보류” 같은 의미 차이는 별도 평가',FONT['label'])
end('docs/assets/wiki-query-ranking.svg')
