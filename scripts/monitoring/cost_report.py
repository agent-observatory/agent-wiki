...
LIMITS = {'cpu': Decimal('1500'), 'memory': Decimal('9000')}
LABELS = {'cpu': 'A1 CPU', 'memory': 'A1 메모리', 'block': '블록 스토리지',
          'object': '오브젝트 스토리지', 'network': '외부 전송', 'other': '기타'}
    ...
    for currency, total in cost_totals(snapshot['cost_month']).items():
        if total > 0:
            ...
            alerts.append((f'cost:{currency}', bracket,
                           f'청구액 발생 · {currency} {amount(total)}',
                           '무료 운영 목표를 벗어난 비용 항목을 Cost Analysis에서 확인하세요.'))
    usage = group_usage(snapshot['usage_month'])
    for c, used in cap_values(usage).items():
        ratio = used / LIMITS[c]
        if ratio >= Decimal('.8'):
            band = math.floor(ratio * 10)
            alerts.append((f'cap:{c}', band, f'{LABELS[c]} 보수적 월 예산 {ratio:.1%}',
...
def daily_message(s, test=False):
    ...
    caps = cap_values(usage)
    status = '비용 확인 필요' if any(v > 0 for v in costs.values()) else '수신된 비용 0' if costs else '집계 대기'
    blocks = [header(('🧪 [시험 전송] ' if test else '📊 ') + 'Agent Wiki · 비용과 사용량'),
              {'type': 'section', 'fields': [
                  {'type': 'mrkdwn', 'text': f"*이번 달 비용*\n{money(costs)}"},
                  {'type': 'mrkdwn', 'text': f"*집계 상태*\n{status} · 추가 반영 중"}]},
              {'type': 'divider'},
              section('*A1 월 사용량 · 보수적 무료 예산*'),
              {'type': 'section', 'fields': [
                  {'type': 'mrkdwn', 'text': f'*{LABELS[c]}*\n' + (
                      f'{amount(caps[c])} / {amount(LIMITS[c])} ({caps[c]/LIMITS[c]:.1%})'
                      if c in caps else '미집계')}
                  for c in ('cpu', 'memory')]}, {'type': 'divider'}]
    ...
    ordered = sorted(usage, key=lambda k: (list(LABELS).index(category(k)), k))
    fields = []
    for k in ordered[:6]:
    ...
    blocks += [{'type':'divider'}, section(f"*어제 {s['yesterday_date']} · 잠정*  {money(cost_totals(s['yesterday_cost']))}\n{comparison}"),
               {'type':'context', 'elements':[{'type':'mrkdwn','text':
                f"조회 {s['checked_at']} · 집계 날짜 UTC · 최대 48시간 지연\n미집계는 0이 아니며, 무료 여부는 모든 항목 반영 후 확인합니다."}]}, links()]
...
def main():
    ...
    parser.add_argument('--mode', choices=['check', 'daily', 'preview'], default='preview')
